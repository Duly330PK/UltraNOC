...
import { BarChart2, Compass, Shield, Search, LogOut, TerminalSquare, Box, HardDrive, Moon, Sun } from 'lucide-react';
import GlobalSearch from '../components/shared/GlobalSearch';

const navItems = [
    { path: '/dashboard', label: 'Dashboard', icon: <BarChart2 size={18} /> },
    { path: '/topology', label: '2D-Topologie', icon: <Compass size={18} /> },
    // { path: '/topology-3d', label: '3D-Topologie', icon: <Globe size={18} /> }, // Removed to prevent errors
        ...
        <div className="flex h-screen bg-lm-bg text-lm-text dark:bg-noc-dark dark:text-noc-text">
            <aside className="w-64 bg-lm-bg-secondary dark:bg-noc-light-dark flex-shrink-0 flex flex-col border-r border-lm-border dark:border-noc-border">
                ...
                <div className="px-2 py-4 border-t border-lm-border dark:border-noc-border space-y-2">
                    <button onClick={toggleTheme} className="w-full flex items-center gap-3 px-3 py-2 text-lm-text-secondary dark:text-noc-text-secondary rounded-md text-sm font-medium hover:bg-lm-border dark:hover:bg-noc-border transition-colors duration-200">
                        {theme === 'dark' ? <Sun size={18} /> : <Moon size={18} />}
                        <span>{theme === 'dark' ? 'Heller Modus' : 'Dunkler Modus'}</span>
                    </button>
                    <button onClick={toggleCommandMenu} className="w-full flex items-center gap-3 px-3 py-2 text-lm-text-secondary dark:text-noc-text-secondary rounded-md text-sm font-medium hover:bg-lm-border dark:hover:bg-noc-border transition-colors duration-200">
                    ...
                    <button onClick={logout} className="w-full flex items-center gap-3 px-3 py-2 text-lm-text-secondary dark:text-noc-text-secondary rounded-md text-sm font-medium hover:bg-noc-red hover:text-white transition-colors duration-200">
                        <LogOut size={18} />
                        <span>Logout</span>
                    </button>
                </div>
